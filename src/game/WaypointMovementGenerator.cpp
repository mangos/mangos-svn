...
WaypointMovementGenerator::_load(Creature &c)
{
    ...
    if( result )
    {
        ...
        do
        {
            ...
            if(!MaNGOS::IsValidMapCoord(i_path[count].x)||!MaNGOS::IsValidMapCoord(i_path[count].y))
            {
                ...
                i_path[count].z = MapManager::Instance ().GetMap(c.GetInstanceId())->GetHeight(i_path[count].x,i_path[count].y);
            }
