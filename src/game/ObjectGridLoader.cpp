...
ObjectGridLoader::Visit(std::map<OBJECT_HANDLE, GameObject *> &m)
{
    ...
    QueryResult *result = sDatabase.PQuery(
        "SELECT `id`,`gameobject`.`map`,`gameobject`.`position_x`,`gameobject`.`position_y`,`position_z`,`orientation`,`rotation0`,`rotation1`,`rotation2`,`rotation3`,`loot`,`respawntimer`,`gameobject`.`guid`,`animprogress`"
