        ...
        if(f) {
            ...
            while(fgets(buffer, 500, f)) {
                ...
                if(nameStart > -1 && (iFilterMethod == NULL || (*iFilterMethod)(buffer))) {
                    ++nameStart;
                    int nameEnd = nameStart + findPosChar(&buffer[nameStart], '/', 1); // find the 1. / (now a space only can be found at the end of the name)
                    nameEnd += findPosChar(&buffer[nameEnd], ' ', 1); // find the 1. space (after the name)
