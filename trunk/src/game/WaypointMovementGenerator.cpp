...
WaypointMovementGenerator::_load(Creature &c)
{
    ...
    QueryResult *result = sDatabase.PQuery("SELECT positionx, positiony, positionz, WaitTime1 FROM creatures_mov WHERE creatureid = '%d' ORDER BY id;", c.GetGUIDLow());
...
{
    QueryResult *result = sDatabase.PQuery("SELECT creatureid FROM creatures_mov;");
		...
		if (numOpen != 0)	//if there are still items in the open list
		{
			...
			for (i = 0; i < nodes[atNode].enodenum; i++)	//loop through all the links for this node
			{
				...
				if (nodes[atNode].links[i].flags & PATH_BLOCKED)	//if this path is blocked, skip it
					continue;
				if (bot->client && (bot->client->ps.eFlags & EF_TANK) && nodes[atNode].links[i].flags & PATH_NOTANKS)	//if this path is blocked, skip it
					continue;
