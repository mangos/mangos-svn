...
void HostilReference::updateOnlineStatus()
{
    ...
    if(!isValid())
    {
        Unit* target = ObjectAccessor::Instance().GetUnit(*getSourceUnit(), getUnitGuid());
        if(target)
...
HostilReference* ThreatContainer::selectNextVictim(Creature* pAttacker, HostilReference* pCurrentVictim)
{
    ...
    for(std::list<HostilReference*>::iterator iter = iThreatList.begin(); iter != iThreatList.end() && !found; ++iter)
    {
        ...
        if(pCurrentVictim)
        {
            ...
            if(map->Instanceable() || (!((Creature*)pAttacker)->IsOutOfThreatArea(target))) // The ref hat status online => the place must be accessable  && target->isInAccessablePlaceFor( ((Creature*)pAttacker) ))
            {
