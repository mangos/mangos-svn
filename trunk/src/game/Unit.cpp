...
Unit::~Unit()
{
    ...
    for (uint32 i = 0; i < CURRENT_MAX_SPELL; i++)
    {
        if (m_currentSpells[i]) m_currentSpells[i]->SetDeletable(true); // spell may be safely deleted now
        m_currentSpells[i] = NULL;
    }
...
void Unit::RemoveAura(AuraMap::iterator &i, bool onDeath)
{
    if (IsSingleTarget((*i).second->GetId()))
    {
        if(Unit* caster = (*i).second->GetCaster())
        {
            ...
            sLog.outError("Unit::RemoveAura: cannot remove the single cast aura from the caster, potential crash!");
    }
